...
public class EntityModArrow extends EntityArrow
{
	...
	public void onUpdate()
	{
		...
		Block block = this.worldObj.getBlock(this.xLoc, this.yLoc, this.zLoc);

		if (block.getMaterial() != Material.air)
		{
			block.setBlockBoundsBasedOnState(this.worldObj, this.xLoc, this.yLoc, this.zLoc);
			AxisAlignedBB axisalignedbb = block.getCollisionBoundingBoxFromPool(this.worldObj, this.xLoc, this.yLoc, this.zLoc);
		...
		if (this.inGround)
		{
			int j = this.worldObj.getBlockMetadata(this.xLoc, this.yLoc, this.zLoc);
		...
		else
		{
			...
			for (int i = 0; i < list.size(); ++i)
			{
				...
				{
					f1 = 0.3F;
					AxisAlignedBB area = temp.boundingBox.expand((double) f1, (double) f1, (double) f1);
			...
			if (movingobjectposition != null)
			{
				...
				if (target != null)
				{
					...
					{
						this.xLoc = movingobjectposition.blockX;
						this.yLoc = movingobjectposition.blockY;
						this.zLoc = movingobjectposition.blockZ;
						this.currentBlock = this.worldObj.getBlock(this.xLoc, this.yLoc, this.zLoc);
						this.inData = this.worldObj.getBlockMetadata(this.xLoc, this.yLoc, this.zLoc);
						...
						if (this.currentBlock.getMaterial() != Material.air) this.currentBlock.onEntityCollidedWithBlock(this.worldObj, this.xLoc, this.yLoc, this.zLoc, this);
					}
				}
			}
